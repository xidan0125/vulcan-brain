...
"""
    ...
    async def run_stream(self, user_query: str):
        ...
        while step < MAX_STEPS:
            ...
            # qwen3 原生支持 thinking 字段，不需要标签过滤
            ...
            async for chunk in response_gen:
                ...
                # 分流输出：thinking 和 token 分开
                for (evt_type, output) in filtered_outputs:
                    yield {
                        "type": evt_type,  # 'thinking' 或 'token'
                        "content": output,
                        "is_thinking": evt_type == 'thinking'
                    }
            
            # 流式结束，输出剩余buffer - V2 分流
            final_outputs = filter.flush()
            for (evt_type, output) in final_outputs:
                yield {
                    "type": evt_type,
                    "content": output,
                    "is_thinking": evt_type == 'thinking'
