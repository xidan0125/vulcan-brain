...
for email in db.emails.find(filter_query):
    ...
    if isinstance(ai_extracted, dict):
        ...
        intent = "UNKNOWN"
    stats["intent_with_attachments"][str(intent)] += 1
...
large_jpg = len([s for s in stats["size_dist"].get("jpg", []) if s > 100*1024])
print(f"\n   📌 高价值文档附件: {high_value} 个")
print(f"   📌 大图片(>100KB): {large_png + large_jpg} 个")
print(f"   📌 需视觉模型处理: ~{pdf_count + large_png + large_jpg} 个")
