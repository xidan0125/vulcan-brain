...
"""
V3.8 批量统一提取器 (最终版)
- 按 email_id 聚合：body + 所有附件
- 统一图片加载：images/<stem>/page_*.jpg
"""
import os
import json
import base64
...
def run_batch_extraction(limit: int = None, resume: bool = True):
    ...
    for i, (email_id, att_records) in enumerate(to_extract.items()):
        if (i + 1) % 10 == 0:
            ...
            print(f"\n📊 {i+1}/{len(to_extract)} | ✅{success} ❌{failed} | {rate:.1f}/min")
        ...
        try:
            ...
            print(f"  [{i+1}] {subject}...", end=" ", flush=True)
            ...
            print(f"✅ {fact_count}f {duration:.1f}s")
            ...
            print(f"❌ JSON")
            log = ExtractionLog(email_id, subject, "failed", len(att_records), error=f"json: {e}")
            _write_log(log)
            failed += 1
    ...
    print(f"\n{'='*60}")
    print(f"📊 完成! ✅{success} ❌{failed}")
    print(f"⏱️ 耗时: {(datetime.now() - start_time).total_seconds() / 60:.1f}min")
