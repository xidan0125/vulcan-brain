...
# ============ 图像优化配置 ============
MAX_IMAGE_SIZE = 784  # 最大边长 1024px，平衡清晰度和 token 消耗
...
def run_batch_extraction(limit: int = None, resume: bool = True):
    ...
    for i, (email_id, data) in enumerate(to_extract):
        ...
        try:
            ...
            err_msg = str(e)[:100]
            print(f"      ❌ 失败: {err_msg}")
            log = ExtractionLog(email_id, subject if "subject" in dir() else "", "failed", len(att_records) if "att_records" in dir() else 0, total_size, error=err_msg)
            _write_log(log)
            extracted_ids.add(email_id)
            failed += 1
