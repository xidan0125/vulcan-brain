...
"""
V3.10 批量统一提取器 (优化版)
- max-model-len=49152，可用更大输出空间
- prefix-caching + reasoning-parser 已启用
- vLLM 自动分离 thinking，无需手动解析
"""
import os
import json
import base64
...
# V3.10 优化配置
# max-model-len=49152，reasoning 已分离，输出空间更大
MAX_TOKENS = 40000  # 纯 JSON 输出，无 thinking 开销
MAX_PAGES_PER_ATT = 5
MAX_ATTACHMENTS = 8
RETRY_COUNT = 0  # 不重试，失败的留给 Gemini
REQUEST_TIMEOUT = 600
...
def _do_extract(email_id: str, body: str, attachments: List[Dict]) -> tuple:
    ...
    # V3.10: vLLM reasoning-parser 自动分离 thinking
    # content 直接是 JSON，reasoning_content 是思考过程
    raw = msg.get("content", "")

    # 兼容：如果还有 </think> 标签（reasoning-parser 未生效），手动处理
    if "</think>" in raw:
...
def run_batch_extraction(limit: int = None, resume: bool = True):
    print("=" * 60)
    print("🚀 V3.10 批量统一提取 (优化版)")
    print("=" * 60)
    print(f"配置: max_tokens={MAX_TOKENS}, prefix-caching=ON, reasoning-parser=qwen3")
    print()
