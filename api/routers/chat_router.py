...
async def generate_title_background(session_id: str, user_message: str):
    import logging
    logger = logging.getLogger(__name__)
    
    print(f"[TitleGen] Starting for session {session_id}")
    
    try:
        manager = get_session_manager()
        client = get_llm_client()
        
        prompt = f"Generate a short title (5-15 chars) for: {user_message[:200]}\\nTitle:"
        
        messages = [ChatMessage(role="user", content=prompt)]
        
        print(f"[TitleGen] Calling LLM...")
        ...
        print(f"[TitleGen] LLM returned: {title}")
        
        if len(title) > 30:
            title = title[:30] + "..."
        
        if title:
            result = await manager.update_title(session_id, title)
            print(f"[TitleGen] update_title result: {result}, title: {title}")
    except Exception as e:
        print(f"[TitleGen] ERROR: Failed: {e}", exc_info=True)
...
async def chat_stream(
    ...
    async def generate():
        try:
            ...
            # 7. Auto-generate title on first message
            session = await manager.get_session(session_id)
            import logging
            _logger = logging.getLogger(__name__)
            print(f"[TitleCheck] session_id={session_id}, title={session.get('title') if session else None}, msg_count={session.get('message_count', 0) if session else 0}")
            if session and not session.get("title") and session.get("message_count", 0) >= 2:
                print(f"[TitleCheck] Triggering title generation for {session_id}")
                title = current_user_msg[:30] + ("..." if len(current_user_msg) > 30 else "")
                asyncio.create_task(manager.update_title(session_id, title))
...
async def react_chat_stream_with_history(
    ...
    # 强化工具调用指令 - 防止模型选择不调用工具
    TOOL_ENFORCEMENT = "【重要】当用户要求执行操作（如创建群聊、发送消息、搜索等）时，你必须调用相应的工具来完成，不要直接用文字回复。"
    if messages and messages[0].get("role") == "system":
        messages[0]["content"] += "\n\n" + TOOL_ENFORCEMENT
    else:
        messages.insert(0, {"role": "system", "content": TOOL_ENFORCEMENT})
